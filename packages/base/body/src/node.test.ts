...
import { arrayBuffer, bytes } from "./node.ts";
...
describe("arrayBuffer (node)", () => {
	...
	it("returns an ArrayBuffer with exact byteLength", async () => {
		const result = await arrayBuffer(toStream(fromString("abc")));
